...
const NavIndicadores = ({measurables}) =>{
  ...
  const navButtons = [
    ...
    },
    {
      url: "pir",
      image: PIR,
      selectedImage: PIRSelect,
      indicator: 'PIR'
    }
  ]

  const getActiveClass = (url) => `${urlActual === `/sistema/indicadores/${id}${url && '/'+url}` ? "link__actual" : null}`

  const getActiveImg = (url, img, selImg) => urlActual === `/sistema/indicadores/${id}${url && '/'+url}` ? <img src={selImg} alt="icon" /> : <img src={img} alt="icon" />
